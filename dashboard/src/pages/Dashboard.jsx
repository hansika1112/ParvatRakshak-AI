      ...
      generate a multilingual early-warning message.
              ...
              <div className="risk-analysis">
                ...
                <div className="weather-analytics">
                  ...
                  <div className="weather-analytics-header">
                    <div>
                      <h3>📈 Weather Trend & Risk Analytics</h3>
                      <p>Real daily observations from NASA POWER</p>
                    </div>

                    <span className="weather-data-badge">
                      {riskAnalysis.weather.daily?.filter(
                        (item) => item.rainfall != null
                      ).length || 0} valid days
                    </span>
                  </div>

                  {(() => {
                    const dailyWeather = (riskAnalysis.weather.daily || [])
                      .filter((item) => item.rainfall != null)
                      .slice(-14);

                    if (!dailyWeather.length) {
                      return (
                        <div className="weather-analytics-empty">
                          No daily rainfall trend data available.
                        </div>
                      );
                    }

                    const maxRain = Math.max(
                      ...dailyWeather.map((item) => Number(item.rainfall) || 0),
                      1
                    );
                    ...
                    return (
                      <>
                          ...
                          {dailyWeather.map((item) => {
                            ...
                            return (
                              <div
                                ...
                                key={item.date}
                                title={`${item.date}: ${rainfall.toFixed(2)} mm`}
                              >
                                <div className="rainfall-bar-value">
                                  {rainfall >= maxRain * 0.65
                                    ? `${rainfall.toFixed(1)}`
                                    : ""}
                                </div>

                                <div className="rainfall-bar-track">
                                  <div
                        ...
                        <div className="weather-analytics-summary">
                        ...
                        <div className="weather-risk-indicator">
                          <div>
                            <span>Rainfall Risk Signal</span>
                            <strong>
                              {Number(riskAnalysis.weather.rainfall_7d) >= 100
                                ? "Elevated"
                                : Number(riskAnalysis.weather.rainfall_7d) >= 50
                                  ? "Moderate"
                                  : "Lower"}
                            </strong>
                          </div>
                          ...
                          <small>
                            Based on recent 7-day rainfall accumulation.
                            This is a prototype screening indicator, not an
                            official warning.
                          </small>
                        </div>

                        <div className="weather-chart-footer">
                          <span>
                            Showing last {dailyWeather.length} valid rainfall
                            observations
                          </span>
                          <span>
                            Latest valid data: {riskAnalysis.weather.latest_date}
                          </span>
                        </div>
                      </>
                    );
